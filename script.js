        ...
        var categories =  Object.keys(this.props.data.practices).map(function (category) {
            var capabilityRows = this.props.data.practices[category].map(function(capability) {
                ...
                var cols = [0,1,2].map(function (i) {
                    return <td><input type="checkbox" name="level" checked={capability.level == i} onChange={this.levelChange.bind(this,i,capability,capability.level != i)}/></td>;
                },this);
                return <tr className={mapLevelsToStyles[capability.level]}><td>{capability.label}</td>{cols}</tr>
            },this);
            return [].concat.apply([], [[<tr><th colSpan="4">{category}</th></tr>], capabilityRows]);
        },this);
        return <table className="table table-hover"><tr><th></th><th>Poor</th><th>OK</th><th>Great</th></tr>{categories}</table>;
    },
...
React.render(<Table data={capabilities} onInput={input} />, document.getElementById('app'));
